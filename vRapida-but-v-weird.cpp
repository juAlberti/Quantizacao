...
unsigned int ContadoresRGB[256][256][256];//preto(0,0,0)
...
void buscarCoresFreq() {
    for (int i = 0; i < 50; i++)
    {
        ...
        if (insere) {*/
        for (int r = -limiar; r <= limiar; r++) {
                for (int g = -limiar; g <= limiar; g++) {
                    for (int b = -limiar; b <= limiar; b++) {
                        int rR = mr - r;
                        int rG = mg - g;
                        int rB = mb - b;
                        if (rR < 0 || rG < 0 || rB < 0) {
